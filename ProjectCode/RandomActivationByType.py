...
class RandomActivationByType(RandomActivation):
    ...
    def step(self, by_type=True):
        ...
        Args:
            by_type: If True, run all agents of a single breed before running
                      the next one.
        '''
        if by_type:
    ...
    def step_type(self, breed):
        ...
        '''
        agent_keys = list(self.agents_by_type[breed].keys())
        self.model.random.shuffle(agent_keys)
        for agent_key in agent_keys:
            self.agents_by_type[breed][agent_key].step()
